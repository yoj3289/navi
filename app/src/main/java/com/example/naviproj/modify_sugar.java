...
public class modify_sugar {
    ...
    public modify_sugar(Context context, DialogCloseListener listener) {
    ...
    public void showDialog() {
        ...
        changeText(); //input_sugar에서 사용하는 xml디자인 그대로 쓰고 텍스트만 바꿔주기 위한 코드
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (inputDate.getText().toString() != "") {
                    ...
                    if (cursor.moveToFirst()) {
                        inputSugar.setText(cursor.getString(0));
                        inputSugar.setEnabled(true);
                    } else {
                        inputSugar.setEnabled(false);
                        inputSugar.setText("수정할 데이터가 없어요!");
                    }
                    db.close();
                    cursor.close();
                }
    ...
    private void insertDataToUserPressTable(String userName, String userDate, String userTime, Integer userSugar) { //데이터를 추가하기 위한 알고리즘
        ...
        String query = "UPDATE userSugar SET BloodSugar=? WHERE (Date=? AND Time=?)";//이렇게 변수를 직접 전달하지 말고 ?로 전달하여 SQLinjection방지
        db.execSQL(query, new Object[]{userSugar,userDate,userTime});
        db.close();
